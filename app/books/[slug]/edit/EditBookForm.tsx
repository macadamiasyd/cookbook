...
async function compressForUpload(file: File): Promise<Blob> {
  ...
  for (const [maxDim, quality] of [[1600, 0.85], [1280, 0.8], [1024, 0.75], [800, 0.7]] as const) {
  ...
  async function uploadPendingFile(): Promise<string | null> {
    ...
    try {
      ...
      try {
        ...
        if (res.status === 413) throw new Error('Image rejected as too large by the server (>4.5 MB after compression).');
